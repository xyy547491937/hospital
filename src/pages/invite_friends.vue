...
<script>
import projectCard from './shop/project_card'
export default {
  name: 'inviteFriends',
  components: {
    projectCard
  ...
  mounted () {
    ...
    if (this.$store.state.userInfo.custId && this.$store.state.userInfo.mobile) {
      ...
      this.$router.push('/pages/authorization')
    }
    switch (this.inviteActive) {
      case 1:
        this.infoText = '邀请好友参团'
  ...
  onShareAppMessage () {
    return {
      ...
      path: `pages/invite_friends?inviteActive=2&id=${this.id}&commodityId=${this.commodityId}`,
