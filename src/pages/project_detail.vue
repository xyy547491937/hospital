<template>
    ...
    <div v-if="groupState == '拼团中'" class="join_list after_line">
      ...
      <div>
        <div v-if="index < 2" v-for="(item, index) of joinList" :key="index" class="spell_details_join_items">
            ...
            <div style="display: flex;flex: 1;justify-content: flex-end;align-items: center;">
                <div style="display: flex;flex-direction: column;align-items: flex-end;margin-right: .17rem;">
                    <span>还差<span style="color: #FF4691;">{{ item.gapNo }}人</span>拼成</span>
                    <span style="color: #aaa;">剩余{{ item.time }}</span>
                </div>
                <div>
                    <button @click="go(index)" class="spell_btn">去拼单</button>
                </div>
            </div>
        </div>
      </div>
      ...
      <!-- <i-cell :title="doctor.personalName" :value="doctorInfo" is-link></i-cell> -->
		...
		<div class="box box1">
			...
			<div class="tab-item" v-show="orderTabIndex==0">
				...
				<rich-text :nodes="comm.commInfo"></rich-text>
          ...
          {{ doctor.commInfo }}
				...
				<div class="item-block">
					...
					<rich-text :nodes="doctor.introduce"></rich-text>
				...
				<rich-text :nodes="organ.introduce"></rich-text>
				<div class="tab-title">版权声明</div>
				<div class="item-block">
					<p>如在胚豆发布的“胚豆网上内容”设计知识产权争议等问题，请相关权利人或利害关系人与胚豆网联系。</p>
				</div>
				<div class="tab-title">免责声明</div>
				<div class="item-block">
					<p>任何信息都不能替代职业医师面对面的诊断和治疗，本网站所转载的内容信息和数据仅供参考。</p>
				</div>
			</div>
		...
		<div class="footer">
				<div class="footer-left">
						<div @click="toCall">
              <img src="/static/images/shop/call.png" alt="">
              <span>电话</span>
						</div>
            <div class="customer_box">
              <img src="/static/images/shop/message_b.png" alt="">
              <!-- <span>客服</span> -->
      ...
      <div class="modal_main">
        <p class="modal_title">
          ...
          <span @click="cancelModal">
            <img class="modal_cancel" src="/static/images/shop/cancel.png" alt="">
          </span>
        </p>
        <scroll-view scroll-y style="height: 600rpx;" @scrolltolower="lower">
          <div v-for="(item, index) of joinList" :key="index" class="spell_details_join_items">
              <div style="display: flex;align-items: center;">
                  <img class="spell_details_join_item modal_img" :src="item.icon">
                  <p style="margin-left: .17rem;font-size: 26rpx;">{{ item.nickName }}</p>
              </div>
              <div style="display: flex;flex: 1;justify-content: flex-end;align-items: center;">
                  <div style="display: flex;flex-direction: column;align-items: flex-end;margin-right: .17rem;font-size: 22rpx;">
                      <span>还差<span style="color: #FF4691;">{{ item.gapNo }}人</span>拼成</span>
                      <span style="color: #aaa;">剩余{{ item.time }}</span>
                  </div>
                  <div>
        ...
        <p class="modal_footer">仅显示10个正在拼单的人</p>
      </div>
    </div>
    ...
    <div v-if="modal2" class="modal">
      <div class="modal2_main">
        ...
        <div class="modal2_imgs">
          <img class="modal2_img" :src="joinList[nowIndex].icon" alt="">
          <img class="modal2_img" src="/static/images/shop/other.png" alt="">
        </div>
	...
	export default {
		...
		methods: {
			...
			jump(val){
				...
				this.modal2 = false
				if (!val) {
					this.$router.push({path:"/pages/submit_order", query: {
				...
				this.$http.post('commodity/queryOneGroupCommidityByPk', {
					...
					if (res.code === 1) {
						...
						if (this.comm.commInfo) {
							this.comm.commInfo = this.dealRichP(this.dealRichText(this.comm.commInfo))
						}
						if (this.organ.introduce) {
							this.organ.introduce = this.dealRichP(this.dealRichText(this.organ.introduce))
						}
						if (this.doctor.introduce) {
							this.doctor.introduce = this.dealRichP(this.dealRichText(this.doctor.introduce))
						}
