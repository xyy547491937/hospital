    ...
    <div class="inputNumber">
        <!-- 数量加加 -->
          <div class="c-num " v-if="addNum">
            <span class="add-num" @click="addNumc(-1)">-</span> <input type="number" :value="num" v-model="num">
          ...
          <div class="sel " v-if="sel" >
              <p class="flex-r co" @click="isShowType(0)"> 
                  ...
                  <span class="c" :class="{'active-font':isShow==0}"> 全款付： <span> ￥{{comm.rulePrice }}</span></span>
             
              </p>
              <p class="flex-r co" @click="isShowType(1)">
                  <span class="c er-circle" :class="{'cart-active':isShow==1}" ></span>
                  <span class="c" :class="{'active-font':isShow==1}"> 预付定金 <span class="c"> ￥{{comm.depositPrice}}</span> <span class="c" style="margin-left:10px;color:#999;"> 到院再付：<span class="c" >￥{{comm.rulePrice-comm.depositPrice}}</span></span></span>
              </p>
          </div>
    </div>
</template>
