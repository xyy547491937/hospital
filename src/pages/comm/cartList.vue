    ...
    <div>
      ...
      <div class="cart" v-for="(item,i) in cartArr" :key="item" :index="i">
        ...
        <div class="cart-title flex">
          <!-- 选择按钮 -->
          <span class="er-circle er-circle-organ" :class="{'cart-active':item.isShopChecked}" @click="selectShop(item)"
            v-if="isSelect"></span>
        ...
        <div class="c" v-if="!isCartShow">
          <div class="cart-body flex" v-for=" v in item.cartList" :key="v">
            ...
            <div class="no-sle flex" v-if="isSelect">
              <span class="er-circle" :class="{'cart-active':v.isChecked}" @click="singleChecked(item,v)"> </span>
            </div>
            <img :src="v.commImg" :alt="v.commName">
            <div class="cart-body-title">
              <p class="mui-ellipsis-2"> <span>[{{v.commName}}］{{v.remark}}</span></p>
              <p class="all-money"><span>{{v.buyType==0?'定金付':"全款付"}}</span> <span style="margin-left:10px; ">¥{{v.oldPrice
                  }}</span> <span style="color:#999;margin-left:10px;" v-if="item.buyType==0">到院再付：¥{{v.rulePrice-v.oldPrice}}</span></p>
              <div class="server-list flex">
                <div class="icon-box flex">
                  <span class="icon-title" v-if="v.isStage ">分</span>
                  <span class="icon-title" v-if="v.isPrefer">优</span>
                  <span class="icon-title" v-if="v.isRecommend  ">推</span>
                  <span class="icon-title" v-if="v.isHot">热</span>
                  <span class="icon-title" v-if="v.isInsurance ">保</span>
                </div>
                <!-- 引入组件 -->
                <div class="c" v-if="!isnum">
                  x{{v.quantity}}
                </div>
        ...
        <div class="c" v-if="isCartShow">
          <div class="cart-body flex" v-for=" v in item.cartList" :key="v" v-if="v.isChecked">
            ...
            <div class="cart-body-title">
              <p class="mui-ellipsis-2"> <span>[{{v.commName}}］{{v.remark}}</span></p>
              <p class="all-money"><span>{{v.buyType==0?'定金付':"全款付"}}</span> <span style="margin-left:10px; ">¥{{v.buyType==0?v.rulePrice:v.price
                  }}</span> <span style="color:#999;margin-left:10px;" v-if="item.buyType==0">到院再付：¥{{v.rulePrice-v.oldPrice}}</span></p>
              <div class="server-list flex">
                <div class="icon-box flex">
                  <span class="icon-title" v-if="v.isStage ">分</span>
            ...
            that.$http
              ...
              .then(res => {
                ...
                if (res.code == 1) {
                  ...
                  a.forEach((v, i) => {
                    if (v.isShopChecked) {
                      that.cartArr.splice(i, 1);
                    } else {
                      newArr = v.cartList.filter(key => {
