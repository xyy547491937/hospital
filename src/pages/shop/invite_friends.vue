    ...
    <div class="invite_header">
      ...
      <div>
        <div class="header_content">
          <img mode="aspectFit" :src="data.commImg" alt="">
        ...
        <div class="header_main">
          <div>
            <p class="main_title">
              <img mode="aspectFit" class="line" src="/static/images/shop/line_left.png" alt="">
              <span>距结束</span>
              <img mode="aspectFit" class="line line_right" src="/static/images/shop/line_left.png" alt="">
            </p>
            <p class="main_time">
            ...
            <button v-if="inviteActive == 1" open-type="share" class="footer_btn">{{ infoText }}</button>
            <button v-if="inviteActive == 2" @click="joinP" class="footer_btn">{{ infoText }}</button>
            <button v-if="inviteActive == 3" class="footer_btn">{{ infoText }}</button>
...
export default {
  ...
  methods: {
    ...
    getPDetail () { // 获取拼团活动详情和商品详情
      this.$http.post('groupOrder/inviteJoinGroup', {
        id: this.id
      }).then(res => {
        if (res.code === 1) {
          let a = res.data
    ...
    countdown (endTime) {
      let val = endTime.replace(/-/g,"/") // 处理时间格式，兼容ios
  ...
  onShareAppMessage () {
    return {
      ...
      path: `pages/shop/invite_friends?inviteActive=2&id=${this.data.id}&commodityId=${this.data.commodityId}`,
      imageUrl: this.data.commImg
    }
  }
}
</script>
