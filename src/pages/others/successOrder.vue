<template>
  ...
  <div class="confir-order">
      ...
      <div class="content-top">
        ...
        <div class="project" v-if="shopType==2">
          ...
          <div>
            ...
            <p class="price"><span>¥{{comm.rulePrice}}</span><span>¥{{comm.price}}</span></p>
          </div>
        </div>
        <!-- 导入购物车模块 -->
        ...
        <div class="chiose-num chioce-title" v-if="shopType==2">
          ...
          <div class="c-num ">
            <span class="add-num" @click="addNum(-1)">-</span> <input type="number" :value="this.num" v-model="num">
    ...
    <div class="footer">
      <p class="bmoney">实付：<span>¥{{totalMoney}}</span> </p>
...
<script>
import inputNumber from "@/pages/comm/inputNumber";
...
export default {
  ...
  computed: {
    totalMoney() {
      return this.isALLMoney(this.buyType);
    },
    phone() {
  ...
  mounted() {
    ...
    if (wx.getStorageSync("cartData")) {
      this.cartArr = JSON.parse(wx.getStorageSync("cartData"));
    }
  },
  methods: {
    jump() {
      this.$router.push({
        path: "/pages/others/payOrder",
        query: {
    ...
    isALLMoney(n) {
      ...
      if (this.$root.$mp.query.shopType == 2) {
        ...
        this.cartArr.forEach(v => {
          v.cartList.forEach(el => {
            console.log(el);
            money += el.rulePrice * el.quantity;
          });
        });
        console.log(money);
        return money;
    ...
    subOrder(params) {
      ...
      this.$http.post("order/save", params).then(res => {
        if (res.code == 1) {
          this.changeJump("/pages/payOrder", {
            orderId: res.data,
            commType: this.commType,
    ...
    quickSubmit() {
      let params = {
        // remark: "",
        customName: this.defaultAddress.custName,
